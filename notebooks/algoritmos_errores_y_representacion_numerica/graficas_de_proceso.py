...
class GraficaDeProceso:
    ...
    def toLatex(self, ec):
        out = str(printing.latex(ec,mul_symbol='dot'))
        if ("\\mathtt{\\text{" in out):
            out = out[:-2].replace("\\mathtt{\\text{",'')
        out = out.replace("\\{",'{')
        out = out.replace("\\}",'}')
        out = out.replace("\\_",'_')
    ...
    def CpTeorico(self, view=1):
        ...
        self.funcionCpTeorico = sym.lambdify(self.variables[:self.numVariables], self.cpTeorico)           
    ...
    def ErrorTotal(self, view = 1):
        ...
        for i in range(self.numVariables,self.numErrores):
            for j in range(self.numOperaciones,self.numVariables-1,-1): # self.numOperaciones+1   
    ...
    def ErrorRedondeo(self, view = 1, mu = False):
        ...
        self.funcionTe = sym.lambdify(self.variables[:self.numVariables], self.te)       
